...
/**
 * Bulk delete multiple truck types
 * POST /api/truck-types/bulk-delete
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Deactivate all truck types in a single query
    const result = await prisma.resourceType.updateMany({
    ...
    return NextResponse.json(
      {
        ...
        error: {
          code: 'INTERNAL_ERROR',
          message: 'Failed to delete truck types',
        },
      },
      { status: 500 }
    )
